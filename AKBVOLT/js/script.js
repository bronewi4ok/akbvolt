...
// include('./form/_file-input.js');
// include('./form/_select.js');
// include('./form/_validation.js');
// include('./form/_mask.js');
// include('./form/_textarea.js');
// include('./form/_show-pass.js');
// FORM
...
if (spollers.length > 0) {
  for (let index = 0; index < spollers.length; index++) {
    ...
    if (spoller.closest('.spollers').classList.contains('hidden') &&
    ...
    document.addEventListener('keydown', function (e) {
      if (spoller.closest('.spollers').classList.contains('esc') &&
        spoller.classList.contains('spoller_active')) {
        if (e.code === 'Escape') {
          if (spollersGo) {
            spollersGo = false;
    ...
    spoller.addEventListener("click", (e) => {
      ...
      if (spollersGo) {
        ...
        if (spoller.closest('.spollers').classList.contains('one') &&
          ...
          let curentspollers = spoller.closest('.spollers').querySelectorAll('.spoller');
          for (let i = 0; i < curentspollers.length; i++) {
            let curentspoller = curentspollers[i];
            if (curentspoller != spoller) {
              curentspoller.classList.remove('spoller_active');
...
let cookies = () => {
  if (!Cookies.get('cookies') && cookieEl) {
...
cookies();
;
let webp_wrapper = document.querySelector('.wrapper');
// bgi
function testWebP(callback) {
  ...
  webP.onload = webP.onerror = function () {
    ...
    webp_wrapper.classList.add('webp');
  };
}

testWebP(function (support) {

  let bgis = document.querySelectorAll("[data-bgi]");
  if (bgis.length > 0) {
    for (let i = 0, j = bgis.length; i < j; i++) {
      let bgi = bgis[i];
      let bgiData = bgi.getAttribute('data-bgi');
      let bgiLqip = bgi.getAttribute('data-bgi_lqip');
      let bgiDelay = bgi.getAttribute('data-bgi_delay') ? bgi.getAttribute('data-bgi_delay') : 0;

      if (bgiLqip != null) {
        bgi.style.backgroundImage = 'url(' + bgiLqip + ')';
      }

      // if support webp
      if (support && bgiData != null) {
        setTimeout(() => {
          let bgiName = bgiData.replace(/\.[^/.]+$/, "") + '.webp';
          bgi.classList.add('bgi_loaded');
          bgi.style.backgroundImage = 'url(' + bgiName + ')';
        }, bgiDelay);
      }

      // if not support
      else if (bgiData != null) {
        setTimeout(() => {
          bgi.style.backgroundImage = 'url(' + bgiData + ')';
          bgi.classList.add('bgi_loaded');
        }, bgiDelay);
      } else {
        console.log('bgi img or attribute not found');
      }
    }
  }
});
...
let header_dash = document.querySelector('#header__dash');

//  Change header on scroll
function headerScroll() {
  let scrollTop = window.scrollY;
  if (scrollTop > 0) {
    header_dash.classList.add('header__dash_scroll');
  } else if (scrollTop < 1) {
    header_dash.classList.remove('header__dash_scroll');
  }
}
...
let stickyDash;
stickyDash = new hcSticky(header_dash, sticky_configs);
;
// Menu
  ...
  window.addEventListener('load', function () {
    var heroSlider = new Swiper(".hero-slider", {
      ...
      pagination: {
        ...
        bulletActiveClass: 'hero-slider__bullet_active'
      },
    });
...
//REVIEWS SLIDER
// ===================================================================
let reviewsSl = document.querySelector(".reviews-slider");
let reviewsSliderConfigs = {
...
if (reviewsSl) {
  let reviewsSlider = new Swiper(".reviews-slider__container", reviewsSliderConfigs);

  let reviewsMore = document.querySelector('.reviews-slider__more');
  let reviewsBtns = document.querySelectorAll('.reviews-slider__btn');

  reviewsMore.addEventListener("click", function (event) {
    if (reviewsSlider !== undefined) {
      this.classList.add('reviews-slider__more_active');
      reviewsSl.classList.add('reviews-slider_disabled');

      reviewsSlider.destroy();
      reviewsSlider = undefined;
      for (let i = 0; i < reviewsBtns.length; i++) {
        const reviewsBtn = reviewsBtns[i];
        reviewsBtn.classList.add('reviews-slider__btn_hidden');
      }
    } else {
      this.classList.remove('reviews-slider__more_active');
      reviewsSl.classList.remove('reviews-slider_disabled');
      for (let i = 0; i < reviewsBtns.length; i++) {
        const reviewsBtn = reviewsBtns[i];
        reviewsBtn.classList.remove('reviews-slider__btn_hidden');
      }
      reviewsSlider = new Swiper(".reviews-slider__container", reviewsSliderConfigs);
    }
  }, false);
}
...
window.addEventListener('resize', (e) => {
  if (document.body.clientWidth < 1200 && accordion === null) {
    accordion = new Accordion('.accordion');
  } else if (document.body.clientWidth > 1200 && accordion !== null) {
    accordion.destroy();
    accordion = null;
  }
});
...
let headerSearchResults = document.querySelector('.header-search__results');
let headerSearchChip = document.querySelector('.chip');


if (headerSearch) {
  headerSearchResults.addEventListener('click', function (e) {
    e.stopPropagation();
  });

  headerSearchInput.addEventListener('focus', e => {
    headerSearchResults.classList.add('header-search__results_active');

  });

  window.addEventListener('click', (e) => {
    if (e.target != headerSearchResults && e.target != headerSearchInput && e.target != headerSearchResults.children) {
      headerSearchResults.classList.remove('header-search__results_active');
    }
  });
}
;
let pics_slider = document.querySelector('.product-views__pics');
let thumb_slider = document.querySelector('.product-views__thumbs');

if (pics_slider && thumb_slider) {

  // CARD SUBSLIDER
  // ===================================================================
  var productThumbSlider = new Swiper(".product-views__thumbs", {
    watchOverflow: true,
    loop: false,
    speed: 1000,
    spaceBetween: 12,
  ...
  // CARD SLIDER
  // ===================================================================
  var productPicsSlider = new Swiper(".product-views__pics", {
    watchOverflow: true,
    loop: false,
    speed: 1000,
    slidesPerView: '1',
...
;
let callback_form = document.querySelector('#callback');
let popup_done = document.querySelector('.popup_done');
let popup_callback = document.querySelector('.popup_callback');

const form_data = new URLSearchParams(new FormData(callback_form));

callback_form.addEventListener('submit', (e) => {
  e.preventDefault();
